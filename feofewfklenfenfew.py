...
def curly_brace_expr(length):
	...
	if expr_type == 1: # {a}
		...
		expression = "{"+str(random.randrange(10**(length-1),10**(length)))+"}"
		...
		length -= 2
		integer1 = random.randrange(10**(length-1),10**(length)) # length-1 , because we also need the other number to be atleast one char in length
		length -= len(str(integer1))

		print("another length: "+str(length))

		integer2 = random.randrange(10**(length-1),10**(length))
